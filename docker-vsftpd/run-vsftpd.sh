...
if [ "$FTP_USER" = "**String**" ]; then
    export FTP_USER='user1'
fi

# If no env var for FTP_USER2 has been specified, use 'admin':
if [ "$FTP_USER2" = "**String**" ]; then
    export FTP_USER2='user2'
fi
...
# If no env var has been specified, generate a random password for FTP_USER2:
if [ "$FTP_PASS2" = "**Random**" ]; then
    export FTP_PASS2=`cat /dev/urandom | tr -dc A-Z-a-z-0-9 | head -c${1:-16}`
fi
...
mkdir -p "/home/vsftpd/pub"
chmod 555 noboy:nogroup /home/vsftpd/pub
